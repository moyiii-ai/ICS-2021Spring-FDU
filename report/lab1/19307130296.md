...
6. SelectPC计算跳转pc使用的是D阶段的pc，因此开始也将D阶段pc赋初值，但是这样会导致第一条指令被重复执行。实际上第一条指令处于F阶段时必定不会跳转，只需要初始化instrF即可。
...
12. resetn信号不能被用于组合逻辑，为了解决第一条指令被反复执行的问题，将指令内存地址赋值也放入组合逻辑。
